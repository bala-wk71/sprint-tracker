...
export default async function DashboardPage({
  ...
  const priorityCompletionPct =
    totalPriorities > 0 ? Math.round((donePriorities / totalPriorities) * 100) : null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SummaryCard
          ...
          sub={totalTarget > 0 ? `of ${totalTarget.toFixed(1)}h target` : "no target set"}
        />
        <SummaryCard
          label="Tasks"
        ...
        <SummaryCard
          ...
          value={priorityCompletionPct !== null ? `${priorityCompletionPct}%` : "—"}
          sub={
            totalPriorities > 0
      ...
      {!sprint ? (
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground">
            Head to{" "}
            <Link href="/sprint/setup" className="text-primary hover:underline">
              Sprint Setup
            </Link>{" "}
      ...
      <section className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="overflow-x-auto">
          <div className="grid min-w-[640px] grid-cols-7 gap-2">
            {dayCells.map((cell) => {
              const isFuture = cell.date > currentWeekStart && cell.date > addDaysIso(currentWeekStart, 6)
                ? false
                : new Date(`${cell.date}T00:00:00`) > new Date();
              const hasData =
                cell.morningMood ||
                cell.closingMood ||
                cell.energy !== null ||
              ...
              return (
                <Link
                  ...
                  className={`rounded-md border p-3 text-xs transition-colors hover:border-primary ${
                    isFuture
